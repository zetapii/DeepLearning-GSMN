...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, bboxes, depends, lengths, ids) in enumerate(data_loader):
        ...
        for i, id in enumerate(ids):
            img_embs[id] = img_emb[i].data.cpu().numpy().copy()

        cap_embs[ids, :max(lengths), :] = cap_emb.data.cpu().numpy().copy()
        # changed - 3rd March
        
        for i, id in enumerate(ids):
            all_bbx[id] = bboxes[i].data.cpu().numpy().copy()
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            im = Variable(torch.from_numpy(
                images[im_start:im_end]), volatile=True).float()
            s = Variable(torch.from_numpy(
                captions[cap_start:cap_end]), volatile=True).float()
            l = caplens[cap_start:cap_end]
            bbx = Variable(torch.from_numpy(
                bbox[im_start:im_end]), volatile=True).float()
...
"""
images: A matrix representing image features (likely extracted from a pre-trained image model).
sims: A matrix of similarities between images and captions. This matrix is likely generated by the model being evaluated, indicating how strongly it connects each image to each caption.
"""
